<template>
  <div>
    ...
    <el-table :data="tableData" @selection-change="handleSelectionChange" border ref="multipleTable" stripe style="width: 100%" tooltip-effect="dark">
      ...
      <el-table-column label="描述" prop="desc" min-width="120"></el-table-column>
      <el-table-column label="作者" prop="author" min-width="120"></el-table-column>
      <el-table-column label="标签" prop="tag" min-width="120"></el-table-column>
      <el-table-column label="编辑时间" prop="updated_at" width="158">
        <template slot-scope="scope">{{scope.row.updated_at | formatDate}}</template>
      </el-table-column>
      <el-table-column label="操作" width="158">
        <template slot-scope="scope">
          <el-button @click="updateArticle(scope.row)" size="small" type="primary" icon="el-icon-edit">编辑</el-button>
    ...
    <!-- 增改表单开始 -->
      <el-dialog :before-close="closeDialog" :visible.sync="dialogFormVisible" :title="dialogTitle" width="640px">
        ...
        <el-form-item label="内容" prop="content" style="height: 380px">
          <quill-editor style="height: 320px" ref="myQuillEditor" v-model="formData.content"></quill-editor>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteArticle(JSON.stringify(ids))
        if (res.code == 200) {
          this.$message({
            type: 'success',
            message: '批量删除成功'
    ...
    async updateArticle(row) {
      const res = await findArticle(row.id);
      this.type = "update";
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          ...
          break;
        case "update":
          res = await updateArticle(this.formData.id, this.formData);
          break;
        default:
          res = await createArticle(this.formData);
          break;
