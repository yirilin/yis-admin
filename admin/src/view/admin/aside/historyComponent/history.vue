<template>
  <div class="router-history">
    <el-tabs :closable="!(historys.length==1&&this.$route.name=='home')"
             @contextmenu.prevent.native="openContextMenu($event)"
             @tab-click="changeTab"
             @tab-remove="removeTab"
             type="card"
             v-model="activeValue">
      <el-tab-pane :key="item.name" :label="item.meta.title" :name="item.name" :tab="item" v-for="item in historys">
      </el-tab-pane>
    </el-tabs>
    <!--自定义右键菜单html代码-->
...
export default {
  ...
  methods: {
    openContextMenu(e) {
      if (this.historys.length == 1 && this.$route.name == 'home') {
        return false
      }
      if (e.srcElement.id) {
